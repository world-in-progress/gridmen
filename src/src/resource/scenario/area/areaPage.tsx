...
import { Grid3x2, MapPin, Save, X } from 'lucide-react'
...
import { addMapLineBetweenPoints, addMapMarker, addMapPatchBounds, adjustPatchBounds, calculateGridCounts, clearDrawPatchBounds, clearGridLines, clearMapMarkers, convertSinglePointCoordinate, convertToWGS84, startDrawingRectangle, stopDrawingRectangle } from '@/components/mapContainer/utils'
...
export default function AreaPage({ node }: { node: ISceneNode }) {
    ...
    const handleDrop = async (e: React.DragEvent) => {
        ...
        if (lastElement === 'schemas' && nodeKey) {
            ...
            // 同步更新 schemaEPSG
            if (targetSchema.current?.grid_schema?.epsg) {
                                ...
                                >
                                    {targetSchema.current ? (
                                        <div className="flex items-center justify-center gap-2 border-2 border-gray-300 rounded-lg p-2">
                                            ...
                                            <span className="text-sm font-medium">{targetSchema.current.grid_schema?.name}</span>
